...
static void handle_child( int sig )
{
	pid_t pid;
	int stat;
	//while是为了防止多个子进程同时退出导致信号屏蔽的情况
	while ( ( pid = waitpid( -1, &stat, WNOHANG ) ) > 0 ) {
		/* 对结求的子进程进行善后处理*/
		//也可以不处理，仅仅是为了防止僵尸进程的产生
	}
}
```
如果父进程阻塞在accept调用，记得处理EINTR
...
另外一种使子进程进入僵尸态的情况是：父进程結束或者异常终止，而子进程继续运行.此时子进程的PPID将被操作系统设置为1 ,即init进程.init进程接管了该子进程，并等待它结束。在父进程退出之后，子进程退出之前，该子进程处于僵尸态。 待子进程执行完之后init进程就会回收它
